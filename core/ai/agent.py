...
Инструкции:
    ...
    Формат ответа: Твой ответ ОБЯЗАТЕЛЬНО состоит из двух частей, разделенных маркером ---:
        Часть 1: Твое сообщение пользователю (текст).
        Часть 2: Технические данные для системы (строго JSON).'''
)
agent = create_agent(
    model=model,
    tools=tools,
